...
public class BedRoomAll extends Application {
...
//        roomGroup.getChildren().add(chair);
        ...
        chair.setTranslateZ(-170); // Adjust position as needed
        ...
        scene.setOnMousePressed(event -> {
            bed.setMouseStart(event.getSceneX(), event.getSceneY());
        });

        scene.setOnMouseDragged(event -> {
            bed.rotateBed(event.getSceneX(), event.getSceneY());
        });
        ...
        scene.setOnMousePressed(event -> {
            ...
            isDragging = bed.getBoundsInParent().contains(event.getX(), event.getY());
        });

        scene.setOnMouseDragged(event -> {
            double deltaX = event.getSceneX() - mouseX;
            double deltaY = event.getSceneY() - mouseY;
            bed.adjustTranslation(deltaX, deltaY, 0);
            mouseX = event.getSceneX();
            mouseY = event.getSceneY();
        });
...
//        });
        ...
        scene.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.LEFT) {
                bed.rotateBedNode(0, -10, 0); // Rotate chair left
            } else if (event.getCode() == KeyCode.RIGHT) {
                bed.rotateBedNode(0, 10, 0); // Rotate chair right
...
//                roomGroup.setRotate(roomGroup.getRotate() + 10); // Rotate room right
                ...
                bed.adjustTranslation(0, 0, -10); // Move chair forward
            } else if (event.getCode() == KeyCode.DOWN) {
                bed.adjustTranslation(0, 0, 10); // Move chair backward
        ...
        primaryStage.setTitle("JavaFX 3D Bed Room");
